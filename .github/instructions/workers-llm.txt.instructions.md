...
https://developers.cloudflare.com/llms.txt
   ...
   - A key reference document for Cloudflare Workers is located at: https://developers.cloudflare.com/workers/llms-full.txt
   - A comprehensive list of other Cloudflare documentation follows the pattern: https://developers.cloudflare.com/[service]/llms-full.txt,
     covering services like Pages, AI Gateway, KV, Vectorize, and the Developer Platform.
<page>
---
...
Totally—here’s the distilled “everything a TypeScript + Wrangler + Workers AI SWE needs” from that `llms-full.txt`, plus battle-ready snippets.

# What to include in your project (the essentials)

* **Wrangler + TS scaffold** via C3, with an **AI binding** on `env.AI`. ([Cloudflare Docs][1])
* **Model IDs** you’ll actually call (text, code, embeddings, TTS/ASR/images). Keep a short allowlist. ([Cloudflare Docs][2])
* **AI Gateway** (optional but recommended) for caching, analytics, rate-limits, and **OpenAI-compatible** endpoints. ([Cloudflare Docs][3])
* **Streaming responses** (chunked) and **JSON Mode / function calling** patterns where relevant. ([Cloudflare Docs][1])
* Pointers to **more llms-full sources** you can drop into your editor/agent context (Workers, Workers AI, AI Gateway, Vectorize, Workflows). ([Cloudflare Docs][4])
...
[ai]
# Makes env.AI available in your Worker
binding = "AI"

# Optional: Durable objects / KV / D1 / Vectorize can be added here later
# [[d1_databases]]
# binding = "DB"
# database_name = "app"
# database_id = "<uuid>"
...
(“AI” binding and TS setup flow are straight from the Workers AI quickstart.) ([Cloudflare Docs][1])

---

# TypeScript Worker: chat + streaming + embeddings (+ optional Gateway)

```ts
export interface Env {
  AI: Ai;
  // Optional if you also want Gateway on Workers Binding requests:
  // AI_GATEWAY_ID: string; // set as plain string in code or env var
}
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (pathname === "/embed") {
      // Create embeddings for RAG/vector DB
      const r = await env.AI.run("@cf/baai/bge-m3", {
        text: "Embed this text for semantic search."
      });
      return json(r);
    }

    // Stream a chat completion from a strong, fast general model
    const model = "@cf/meta/llama-3.3-70b-instruct-fp8-fast";

    // OpenAI-style messages are supported when using Gateway compat endpoints,
    // but with Workers Binding we pass {prompt} or {messages} as the model expects.
    const { readable, writable } = new TransformStream();
    const writer = writable.getWriter();
    const enc = new TextEncoder();

    // Kick off inference; with Binding you can also pass a Gateway object for caching
    const stream = await env.AI.run(
      model,
      {
        messages: [
          { role: "system", content: "You are a concise, helpful assistant." },
          { role: "user", content: "Give me a one-sentence summary of Workers AI." }
        ],
        stream: true,            // stream deltas
        // json: true,           // opt-in JSON mode when using models that support it
      },
      {
        // Optional Gateway: cache + analytics (requires an existing gateway)
        gateway: {
          id: "my-gateway",     // or env.AI_GATEWAY_ID
          skipCache: false,
          cacheTtl: 3600
        }
      }
    ...
    (async () => {
      for await (const chunk of stream as AsyncIterable<any>) {
        const content =
          ...
          chunk?.response ??              // some models stream as `response`
          "";
...
(Shows the **AI binding** usage, a **supported model ID**, streaming pattern, and optional **Gateway** binding options.) ([Cloudflare Docs][1])

---

# Handy model IDs to keep in your allowlist

* General chat/reasoning: `@cf/meta/llama-3.3-70b-instruct-fp8-fast`, `@cf/deepseek/deepseek-r1-distill-qwen-32b`
* Coding: `@cf/qwen/qwen2.5-coder-32b-instruct`
* Embeddings: `@cf/baai/bge-m3`
* Vision-to-text: `@cf/llava/llava-1.5-7b-hf`
* ASR: `@cf/openai/whisper-large-v3-turbo`
* Text-to-image: `@cf/black-forest-labs/flux-1-schnell`
  (Names pulled from the current model catalog—verify in the Models page as they evolve.) ([Cloudflare Docs][2])

---

# OpenAI-compatible option (via AI Gateway)

If you want to reuse OpenAI SDK code:

* Hit Gateway’s **compat** endpoint: `/compat/chat/completions`
* Set `"model": "workers-ai/{model_id}"`
  (Supported for chat + embeddings.) ([Cloudflare Docs][3])
...
# (edit wrangler.toml to add [ai] binding)

# Dev (note: AI calls still bill your account)
...
(Exactly the flow in the Workers AI “Workers Bindings” quickstart.) ([Cloudflare Docs][1])

---

# Drop these into your editor/agent “context”

These are the **llms-full** sources Cloudflare recommends for rich context while coding:

* Workers AI models & usage: `https://developers.cloudflare.com/workers-ai/llms-full.txt` ([Cloudflare Docs][2])
* Workers platform (Handlers, DOs, WebSockets, runtime APIs): `https://developers.cloudflare.com/workers/llms-full.txt` (linked from Prompting page) ([Cloudflare Docs][4])
* AI Gateway (analytics, caching, OpenAI-compat): `https://developers.cloudflare.com/ai-gateway/llms-full.txt` (page lists compat details) ([Cloudflare Docs][3])
* Vectorize (for RAG): `https://developers.cloudflare.com/vectorize/llms-full.txt` ([Cloudflare Docs][5])
* Workflows (durable, long-running, retriable orchestration): `https://developers.cloudflare.com/workflows/llms-full.txt` ([Cloudflare Docs][6])

Tip from Cloudflare: pasting `…/llms-full.txt` URLs into editors like Cursor/Zed/Windsurf gives the model full docs context while it generates code. ([Cloudflare Docs][4])

---

If you want, I can tailor the snippet to your exact stack (Vectorize index init + D1 logging + Guardrails via Gateway) and drop in a ready-to-deploy repo skeleton.
...
[2]: https://developers.cloudflare.com/workers-ai/models/ "Models · Cloudflare Workers AI docs"
[3]: https://developers.cloudflare.com/ai-gateway/providers/workersai/ "Workers AI · Cloudflare AI Gateway docs"
[4]: https://developers.cloudflare.com/workers/get-started/prompting/ "Prompting · Cloudflare Workers docs"
[5]: https://developers.cloudflare.com/vectorize/llms-full.txt?utm_source=chatgpt.com "Vectorize llms-full.txt"
[6]: https://developers.cloudflare.com/workflows/?utm_source=chatgpt.com "Overview · Cloudflare Workflows docs"


Great Q. TL;DR in 2025 you should **stop hand-installing `@cloudflare/workers-types` for your runtime** and instead let Wrangler **generate types that exactly match your Worker config**.

Here’s the clean setup I recommend:

# Use Wrangler-generated types (preferred)

1. Generate types from your `wrangler.toml/jsonc`:

```bash
pnpm wrangler types
# creates: worker-configuration.d.ts (includes Env + runtime APIs)
```

2. Point TS at the generated file:
...
3. (If you use `nodejs_compat`) also install Node types and include them:
...
Cloudflare’s current docs recommend this flow and explain why it’s better (types are derived from your **compat date/flags + bindings**, so they’re always accurate). ([Cloudflare Docs][1])

# What you get

* A strongly-typed `Env` that mirrors your bindings (KV/D1/R2/Vectorize/**AI** etc). For example, with `[ai] binding = "AI"` you’ll get:

```ts
export interface Env {
  AI: Ai; // comes from the generated .d.ts
}
```

* Runtime API types that respect your compatibility date/flags (e.g., `nodejs_als`, `nodejs_compat`). ([Cloudflare Docs][1])

# Add it to scripts/CI

Run `wrangler types` before type-checking/build so the file stays fresh:

```jsonc
{
  "scripts": {
    "generate-types": "wrangler types",
    "type-check": "pnpm run generate-types && tsc -p tsconfig.json"
  }
}
```

Cloudflare suggests re-generating after any config change. ([Cloudflare Docs][1])

# When to still use `@cloudflare/workers-types`

* **Libraries/shared packages** that target the Workers runtime (not your deployed Worker) can keep `@cloudflare/workers-types`. For your actual Worker, prefer `wrangler types`. The docs call this out explicitly. ([Cloudflare Docs][1])

# Pages Functions variant

If you’re on Pages Functions, you can drop the types file next to `/functions`:

```bash
pnpm wrangler types --path="./functions/types.d.ts"
```

…and include that path in `tsconfig.json`. ([Cloudflare Docs][2])

# Gotchas (and fixes)

* **Vitest / `@types/node` conflicts**: adding `node` types globally can fight the web-standard runtime. Keep `"types": ["./worker-configuration.d.ts"]` minimal; only add `"node"` if you’ve enabled `nodejs_compat`. Known reports mention friction with test setups; scoping types and running `wrangler types` first typically resolves it. ([GitHub][3])
* **Older Wrangler**: if you’re on >3.66.0 and <4.0.0 you may see the historical `--experimental-include-runtime` note; otherwise the current flow above is the one to use. ([Cloudflare Docs][1])

If you want, paste your `wrangler.toml` and I’ll generate a ready-to-commit `worker-configuration.d.ts`, the exact `Env` shape (including `AI`), and a minimal `tsconfig.json` tuned for your stack.
...
[2]: https://developers.cloudflare.com/pages/functions/typescript/?utm_source=chatgpt.com "TypeScript · Cloudflare Pages docs"
[3]: https://github.com/cloudflare/workers-sdk/issues/8802?utm_source=chatgpt.com "Wrangler types is recommended but hard to actually use in ..."
